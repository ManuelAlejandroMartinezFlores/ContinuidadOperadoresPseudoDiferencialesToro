...
\begin{definition}[Espacios de Sobolev]
	...
	\end{equation*} 
\end{definition}
Ahora, se extiende el resultado del Teorema \ref{theo:L2-boundedness-S0} para estos espacios. 
\begin{theorem}
	Sea $T\in\Psi^m_{1,0}(\R^n\times\R^n)$ sea un operador pesudo-diferencial de orden $m\in\R$. Entonces, el operador $T$ se extiende a un operador continuo desde el espacio de Sobolev $W^{s-m}_2(\R^n)$ hacia $W^s_2(\R^n)$. 
\end{theorem}
\begin{proof}
	Note que $J^{s-m}TJ^{-s}$ es un operador de orden cero, por lo que es continuo en $L^2$, en vista del Teorema \ref{theo:L2-boundedness-S0}. Por lo que se tiene que 
	\begin{equation*}
		\|f\|_{W^{s-m}_2} = \|J^{s-m}Tf\|_{L^2} = \|J^{s-m}TJ^{-s}J^sf\|_{L^2} \lesssim \|J^sf\|_{L^2} = \|f\|_{W^s_2}.
	\end{equation*}
	Completando la prueba.
\end{proof}
